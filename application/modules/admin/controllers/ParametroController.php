...
class Admin_ParametroController extends Admin_IndexController
{
	private $_parametro = null;
	private $_services_simple_crud = null;

	public function init()
	{
		$this->_helper->layout->disableLayout();
		$this->_parametro = new Domain_Parametro();
		$this->_services_simple_crud = new Services_SimpleCrud();
	}

	public function indexAction()
	{
		$this->_forward('list','parametro','admin');
	}

	public function listAction(){
			
		$service_model = new Services_SimpleCrud();
		$rows = $this->_services_simple_crud->getAll($this->_parametro->getModel());
		
		$page=$this->_getParam('page',1);
		
		//$a = Domain_Parametro::getParametroByNombre('email_from');
...
public function saveMenuPerfilAction()
	...
	public function addAction()
	{
		...
		if( isset($params['add']) ){
			...
			$this->_services_simple_crud->save($this->_parametro->getModel(),$values);
		}
	}
